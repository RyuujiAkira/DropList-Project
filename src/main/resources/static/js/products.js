...
let createCardProducts = () => {
    ...
    newTitle.textContent = document.getElementById('productName').value;
...
let postData = () => {
    ...
    let productBrandIDValue = productBrandID.value;
    let releaseDateValue = releaseDate.value;

    let newObject = {
        productName: productNameValue,
        productType: productTypeValue,
        productBrandID: productBrandIDValue,
...
let deleteRequest = (brandID) => {
    ...
    fetch(`https://localhost:8080/item/${id}`, {
        method: `DELETE`
    })
        .then((response) => {
...
submitBtn.addEventListener('click', createCardProducts);
